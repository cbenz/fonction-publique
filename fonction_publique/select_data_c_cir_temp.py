...
def select_ident(dataset):
    variable =  'c_cir'
    c_cir =  get_careers(variable = variable, data_path = dataset)
    subset_by_corps = {}
    for corps in ['AT', 'AA', 'AS']:
        grilles = select_grilles(corps = corps)
        list_code = list(set(grilles.code_grade_NETNEH.astype(str)))
#        list_code = ['0' + s for s in list_code]
        subset_ident = list(set(c_cir.ident[c_cir.c_cir.isin(list_code)]))
