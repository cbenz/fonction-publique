...
"Il y a {} agents qui sont ATT en 2011 selon leur c_cir".format(
...
#        print "En supprimant les individus dont le c_cir ou l'échelon est nul en {}, on a {} agents".format(annee, len(data.ident.unique()))
#
...
#            ]].drop_duplicates()
    ...
    if annee == 2011:
        ...
        data_with_date_effet_grille = data.merge(df_corres,
                  on = ['c_cir_{}_predit'.format(annee), 'annee', 'date_effet_grille_{}'.format(annee)], how = 'left')

        grilles['c_cir_{}_predit'.format(annee)] = grilles['c_cir'] # TOFIX
        grilles['date_effet_grille_{}'.format(annee)] = grilles['date_effet_grille']
        grilles['ib_{}'.format(annee)] = grilles['ib']

        data_with_info_grilles = data_with_date_effet_grille.merge(
            grilles, on = ['date_effet_grille_{}'.format(annee),
                           'c_cir_{}_predit'.format(annee),
                           'ib_{}'.format(annee)], how = 'left'
            )
...
#    print data_t.head()
    ...
    data_ATT = data_ATT.rename(
            columns = {'c_cir':'c_cir_{}'.format(annee),
                       'ib':'ib_{}'.format(annee),
   ...
   # data_t_1 = data_t_1[~data_t_1['ib4'].isin([0, 'NaN', -1])]
    ...
    data_t_1 = data_t_1.rename(columns = {"c_cir":"c_cir_{}".format(annee-1),
                                          "ib":"ib_{}".format(annee-1)})
    data_t_1 = data_t_1[['ident',
...
def get_cas_uniques(data, annee):
    ...
    cas_uniques = data[["c_cir_{}".format(annee),
                        "ib_{}".format(annee-1),
                        "date_effet_grille_{}".format(annee)
                        ]].drop_duplicates(["c_cir_{}".format(annee),
                         "ib_{}".format(annee-1)])
