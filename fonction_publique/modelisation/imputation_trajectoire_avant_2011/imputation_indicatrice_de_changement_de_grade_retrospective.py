...
def impute_indicatrice_chgmt_grade(data_cas_uniques, transit_intra_corps, annee):
    ...
    for cas in range(len(cas_uniques)):
        ...
        date_effet_grille_now = pd.to_datetime(
                        cas_unique['date_effet_grille_{}'.format(annee)])
        ...
        date_effet_grille_bef = grilles[(grilles['code_grade_NETNEH'] == c_cir_now) &
                (grilles[
                        'date_effet_grille'
                        ] <= pd.datetime(annee-1, 12, 31))].date_effet_grille.max()
        ...
        if annee == 2009:
            ...
            list_date_effet_grille_bef = [
                    date_effet_grille_bef
                    ]
        ...
        if len(grille_c_cir_now_inter_ib_bef) == 0:
            ...
            if transit_intra_corps:
                ...
                if annee == 2009:
                    grilles_corps = grilles.query("corps_NETNEH == {}".format(
                            corps_now, annee-1
                            ))
                    list_dates_effet = ['2008-01-07', '2006-01-11']
                    grilles_possibles = grilles_corps[
                            grilles_corps['date_effet_grille'].isin(list_dates_effet)]

                else:
                    grilles_corps = grilles.query(
                            "(corps_NETNEH == {}) & (annee_effet <= {})".format(
                                    corps_now, annee-1
                                    )
                            )
                ...
                grades_possibles_bef = grilles_possibles.query(
                        'ib == {}'.format(ib_bef)
                        )[['code_grade_NETNEH', 'code_grade_NEG', 'date_effet_grille']].rename(columns=
...
#                    print grades_possibles_bef.head()
                ...
                grades_possibles_bef[
                        "c_cir_{}".format(annee)
                        ] = [c_cir_now] * len(grades_possibles_bef)

                grades_possibles_bef[
                        "ib_{}".format(annee-1)
                        ] = [ib_bef] * len(grades_possibles_bef)
                ...
                grades_possibles_bef[
                        ...
                        ] = (grades_possibles_bef["c_cir_{}".format(annee)] != grades_possibles_bef["c_cir_{}_predit".format(annee - 1)])
                ...
                grades_possibles_bef = grades_possibles_bef.drop_duplicates(
                        grades_possibles_bef.columns.difference(
...
def map_cas_uniques_to_data(cas_uniques_w_indic_df, data, annee, test):
    ...
    data_with_indicatrice_grade_change = data.merge(cas_uniques,
                                                    how = 'outer',
                                                    on = ['c_cir_{}'.format(annee),
                                                          'ib_{}'.format(annee-1)]
                                                    )
    if test:
        data_with_indicatrice_grade_change = data_with_indicatrice_grade_change[['ident',
                                                                             'c_cir_{}'.format(annee),
                                                                             'ib_{}'.format(annee),
    ...
    data_with_indicatrice_grade_change = data_with_indicatrice_grade_change.rename(
            columns = {"date_effet_grille_{}_x".format(annee):"date_effet_grille_{}".format(annee)}
            )
