...
\node (start) [startstop, xshift=-3cm, yshift=-0.4cm] {$ib_0$ = ($C_0, G_0, E_0$)};
\node (choice)[decision, below  of=start,  yshift=3cm] {\shortstack{Sortie\\grade?}};

\node (in1) [process, left of=choice, xshift=1cm] {Sortie du corps?};
...
\node (stop)  [startstop, below  of=choice , yshift=-0.5cm] {$ib_1$ = ($C_1, G_1, E_1$)};
...
\begin{frame}
\frametitle{Prédiction de l'échelon : résultats}
...
\begin{frame}
\frametitle{Prédiction de l'échelon : améliorations}

\begin{itemize}
	\item On fait pour l'instant entrer les agents dans leur échelon de 2011 au dernier trimestre.\\
	$\Rightarrow$ Imputer une durée déjà passée dans l'échelon de 2011
	\item Utiliser des \textbf{moyennes empiriques} de durées passées dans chaque échelon au lieu des durées légales
\end{itemize}
...
\node (in1) [process, right of=start, xshift=2.5cm] {Prochain grade?}
	child { node {Dans le corps} edge from parent node[left,draw=none] {oui}}
	child { node {Hors du corps} edge from parent node[left,draw=none] {oui}}
...
\node (in3) [process, left of=start, xshift=-2.5cm] {$Grade_{t}$ = TTH1?}
child { node {$\Delta$, $Grade_{t-1}$ = autre} edge from parent node[left,draw=none] {oui}}
child { node (start4) [startstop] {$IB_{t-1}$ sur grille $(Grade-1)_{t}$?} 
	child{ node {$\Delta$, $Grade_{t-1}$ = autre} edge from parent node[left, draw=none] {non} }
	child{ node {$\Delta$, $Grade_{t-1}$ = $(Grade-1)_{t}$} edge from parent node[right, draw=none] {oui} }
	edge from parent node[right,draw=none] {non}}
;
